...
set REMOTE_DIR=CelebrityPhotoDownloader
...
:: 建立目標資料夾
echo [2/4] 建立資料夾...
if not exist "%SHARE_PATH%\%REMOTE_DIR%" mkdir "%SHARE_PATH%\%REMOTE_DIR%"

:: 複製檔案
echo [3/4] 複製程式檔案...
copy /y "%~dp0web_app.py" "%SHARE_PATH%\%REMOTE_DIR%\" >nul
copy /y "%~dp0requirements.txt" "%SHARE_PATH%\%REMOTE_DIR%\" >nul
copy /y "%~dp0setup.bat" "%SHARE_PATH%\%REMOTE_DIR%\" >nul
copy /y "%~dp0run_web.bat" "%SHARE_PATH%\%REMOTE_DIR%\" >nul
echo [OK] 檔案已複製
